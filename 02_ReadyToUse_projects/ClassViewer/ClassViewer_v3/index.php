...
if( isset( $_POST['chemin'] ) ){
		...
		if( $fichier_listed != basename ( __FILE__ ) ){
			if (!$fp = fopen($fichier_listed,"r")) {
				...
				if (preg_match_all('#class [a-zA-Z0-9_]*(.*?)//php2uml#s', $Fichier, $out)) {
					...
					echo "Pas de classe dans ce fichier : $value<br/>";
				}
			}
		}
	} 
	if( $nb_file_without_class != 0 )
		echo "N'auriez-vous pas oublié les tag de fin de classe?<br/>Il faut que //php2uml apparaissent à chaque fin de classe.<br/>";
	...
	foreach( $ClassTab as $key => $une_class  ){
		...
		if( preg_match_all('#(public|protected|private) function (([$_a-zA-Z0-9]*)\(.*?\))#', $une_class['class_complete'], $out_func)){
			...
			//	echo "- key $key_func : value ".$out_func[1][$key_func].' '.$out_func[2][$key_func]."<br/>";
				...
				//echo 'Nom Fonction : '.$ClassTab[$key]['fonctions'][$key_func]['fonct_nom']."<br/>";
			} 
		}		
	} 
	...
	// On recherche la/les classe de base qui n'a pas d'héritage.
	$class_base = array();
	$i=0;
