...
require('lang.inc.php');
  ...
  {  
        ...
        echo '</head>'."\r\n".'<body>'."\r\n";
        echo '<br /><br /><br /><br /><div align="center" class="titre1">'.$LANGUE['modules']['config']['accesrf'].'</div>'."\r\n";
        echo '</body>'."\r\n".'</html>';
        exit;
  }
...
if($status !== 1.0){
        ...
        if($fp === false){
                ...
                $buff .= '$CONFIG[\'DETAILS\'] = ' . $_POST['DETAILS'] . ';'."\r\n";
                $buff .= '$CONFIG[\'DIRINFO_LIFE\'] = ' . $_POST['DIRINFO_LIFE'] . ';'."\r\n";

                $buff .= '$CONFIG[\'activer_Message\'] = ' . $_POST['activer_Message'] . ';'."\r\n";

                $buff .= '$CONFIG[\'Message\'] = \'' . $_POST['Message'] . '\';'."\r\n";
                ...
                $buff .= 'if (isset($_SESSION[\'lang\']) && file_exists(dirname(__FILE__).\'/lang/lang.\'.$_SESSION[\'lang\'].\'.ini\'))	$CONFIG[\'SYS_LANG\'] = $_SESSION[\'lang\'];';
...
switch (floor($status)){
        case 1:
                echo $LANGUE['modules']['config']['ERR1'];
                break;
        case 2:
                echo $LANGUE['modules']['config']['ERR2'];
                break;
}
echo "<br>";
switch ((int)(($status-floor($status))*10)) {
        case 0:
                echo $LANGUE['modules']['config']['ERR3'];
                break;
        case 1:
                echo $LANGUE['modules']['config']['ERR4'];
                break;
        case 2:
                echo $LANGUE['modules']['config']['ERR5'];
                break;
}
